...
namespace Leggermente.lib
{
    public class IO
    {
        ...
        public static Variable schermo(Variable var)
        {
            if (var != null)
            {
                if (var.Type == ValueType.Vector)
                {
                    for (int i = 0; i < var.VectLenght; i++)
                    {
                        schermo(var[i]);
                    }
                }
        ...
        public static Variable tastiera()
        {
            return new Variable("_", Console.ReadLine());
        }

        public static Variable tasto(Variable var)
        {
            string r = "";
            int num;
            if (var != null && var.Type == ValueType.Number && int.TryParse(var.Value, out num)) for (int i = 0; i < num; i++) r += Console.ReadKey(true);
            else return new Variable("NULL", null);
            return new Variable("_", r);
        }

        public static Variable tasto()
        {
            return tasto(new Variable("_", "1"));
        }
    }

    public class VAR
    {
        public static Variable tipo(Variable var)
        {
            if (var != null)
            {
                switch (var.Type)
                {
                    case ValueType.Boolean:
                        return new Variable("_", "Boolean");
                    case ValueType.Number:
                        return new Variable("_", "Number");
                    case ValueType.String:
                        return new Variable("_", "String");
                    case ValueType.Vector:
                        return new Variable("_", "Vector");
                    default:
                        return new Variable("_", "Null");
                }
            }
            else return new Variable("NULL", null);
        }

        public static Variable lunghezzaVect(Variable var)
        {
            if (var != null && var.Type == ValueType.Vector)
            {
                return new Variable("_", var.VectLenght);
            }
            else return new Variable("NULL", null);
        }

        public static Variable lunghezzaStringa(Variable var)
        {
            if (var != null && var.Type == ValueType.String)
            {
                return new Variable("_", var.Value.Length);
            }
            else return new Variable("NULL", null);
        }

        public static Variable lunghezzaStringa(Variable var, Variable num1)
        {
            if (var != null && var.Type == ValueType.String && num1 != null && num1.Type == ValueType.Number)
            {
                int numero1;
                if (int.TryParse(num1.Value, out numero1))
                {
                    return new Variable("_", var.Value.Substring(numero1));
                }
                else return new Variable("NULL", null);
            }
            else return new Variable("NULL", null);
        ...
        public static Variable lunghezzaStringa(Variable var, Variable num1, Variable num2)
        {
            if (var != null && var.Type == ValueType.String && num1 != null && num1.Type == ValueType.Number && num2 != null && num2.Type == ValueType.Number)
            {
                int numero1, numero2;
                if (int.TryParse(num1.Value, out numero1) && int.TryParse(num2.Value, out numero2))
                {
                    return new Variable("_", var.Value.Substring(numero1, numero2));
                }
                else return new Variable("NULL", null);
            }
            else return new Variable("NULL", null);
